      ...
      <div className={`hero-div flex md:flex-row flex-col-reverse gap-y-4 gap-x-4 md:justify-between transition-all duration-100 ${showLgaPage ? 'opacity-0 h-0 pointer-events-none' : 'opacity-100 h-auto'}`}>
        ...
        <div className={`md:w-6/12 ${servicebutton ? '' : 'md:mt-20'} md:border-r md:border-gray-400 md:px-12 ${showRequestPage ? 'opacity-0 md:opacity-100 pointer-events-none md:pointer-events-auto' : 'opacity-100'}`}>
          <h1 className="lg:text-7xl text-4xl mt-6 text-gray-900">
            ...
            <p className="leading-8 py-2 lg:py-6">Local Government</p>
            <p className="leading-8 py-2 lg:py-6">Solution</p>
          </h1>
          <p className="mt-2 md:mt-4 pr-6">

The E-Services system - Empowering the heartland, one click at a time. Experience seamless digital governance like never before. Step into the future with our online services designed to make your life easier. Speed, safety, and efficiency—these are not just words, they are our promise.</p>

          <div className="mt-6">
            <button className={`transition-all duration-100 ${servicebutton ? 'opacity-0 hidden' : 'opacity-100' } bg-[#005C30] hover:bg-green-700 text-white w-[200px] h-14 py-4 rounded-md`} onClick={handleservice}>
              Request for Services
            </button>
            <div className={`transition-all duration-200 flex ${servicebutton ? 'opacity-100' : 'opacity-0 pointer-events-none' } gap-x-8`}>
            <button className="bg-[#005C30] hover:bg-green-700 text-white w-[200px] h-14 py-4 rounded-md" onClick={LGAbutton}>
              Select LGA
            </button>

            <button className="bg-[#edbe23] hover:bg-[#c6a025] text-white w-[200px] h-14 py-4 rounded-md" onClick={Requestbutton}>
              Request Status
            </button>
            </div>
          </div>
        ...
        <div className="md:w-5/12 md:mt-12">
          ...
          {showRequestPage && <div className="request-input mt-12 w-full flex md:flex-row flex-col justify-between gap-y-6">
            <input type="text" placeholder="Input your Request Code" className="w-[340px] p-4 outline outline-[#005C30] outline-rounded-md rounded-md"/>
            <button type="submit" className="bg-[#005C30] hover:bg-green-700 w-[200px] text-white rounded-md md:px-4 text-sm lg:text-base h-14 m-auto">Check Request Status</button>
